...
<form method="POST" action="/employee/new" id="employeeForm">
@csrf
      ...
      <div class="col-md-6 d-flex flex-column justify-content-between" style="min-height: 100px;">
        ...
        <div class="d-flex align-items-center gap-3 mt-auto flex-wrap">
          ...
          <div class="text-muted d-flex align-items-center gap-1 cursor-pointer">
            ...
            <a href="/employee/profile/{{ $person->cpf ?? '' }}"><span><strong>Perfil Comportamental</strong></span></a>
          </div>
...
<label class="form-check-label" for="estrangeiro">Estrangeiro</label>
        ...
        <div class="col-md-3">
          ...
          <input value="{{ $person->rg ?? '' }}" name="rg" type="text" class="form-control">
        </div>

        <div class="col-md-3">
          <label class="form-label">CNH</label>
          <input value="{{ $person->cnh ?? '' }}" name="cnh" type="text" class="form-control">
        </div>
        <div class="col-md-3">
          <label class="form-label">PIS</label>
          <input value="{{ $person->pis ?? '' }}" name="pis" type="text" class="form-control">
        </div>
        <div class="col-md-3">
          <label class="form-label">Banco</label>
          <select name="bank" id="bank" class="form-select">
    <option value="{{$person->bank}}">{{$person->bank}}</option>
    @php
        $banks = [
            "001 - Banco do Brasil S.A.",
            "006 - Banco BM&FBOVESPA S.A.",
            ...
            "779 - Banco Topázio S.A.",
            "025 - Banco ABC Brasil S.A.",
            "003 - Banco da Amazônia S.A.",
            "630 - Banco Indusval S.A.",
            "633 - Banco Rendimento S.A.",
            "389 - Banco Mercantil do Brasil S.A.",
            "336 - Banco C6 S.A.",
            "318 - Banco BMG S.A.",
            "094 - Banco BNL do Brasil S.A.",
            "747 - Banco Rabobank International Brasil S.A.",
            "067 - Banco da China Brasil S.A.",
            "746 - Banco Credit Suisse (Brasil) S.A.",
            "273 - Banco Cruzeiro do Sul S.A.",
            "218 - Banco Fiat S.A.",
            "172 - Banco Volkswagen S.A.",
            "249 - Banco GMAC S.A.",
            "248 - Banco Mercedes-Benz S.A.",
            "247 - Banco Honda S.A.",
            "233 - Banco Toyota do Brasil S.A.",
            "607 - Banco Citicard S.A.",
            "643 - Banco Paulista S.A.",
            "623 - Banco Panamericano S.A.",
            "634 - Banco BRJ S.A.",
            "606 - Banco Crefisa S.A.",
            "747 - Banco J.P. Morgan S.A.",
            "476 - Banco Morgan Stanley S.A.",
            "246 - Banco ABN AMRO S.A.",
            "422 - Banco Safra S.A.",
            "756 - Banco Cooperativo do Brasil S.A. - BANCOOB",
            "119 - Banco da Terra S.A.",
            "623 - Banco Pan S.A.",
            "746 - Banco Modal S.A.",
            "212 - Banco Original S.A.",
            "637 - Banco Sofisa S.A.",
            ...
            "077 - Banco Inter S.A.",
            "318 - Banco BS2 S.A.",
            "746 - Banco Digital Modal S.A.",
            "755 - Banco Digio S.A.",
            "026 - Banco Agibank S.A.",
            "212 - Banco Original S.A.",
            "779 - Banco Topázio S.A.",
            "655 - Banco Votorantim S.A."
        ];
        $selectedBank = $person->bank ?? '';
    @endphp

    @foreach ($banks as $label)
        <option value="{{ $label }}" {{ $selectedBank === $label ? 'selected' : '' }}>
            {{ $label }}
        </option>
    @endforeach
</select>
        ...
        <div class="col-md-3">
          ...
          <input value="{{ $person->agency ?? '' }}" name="agency" type="text" class="form-control" placeholder="000000-0">
        </div>

        <div class="col-md-3">
          <label class="form-label">Conta</label>
          <input value="{{ $person->account ?? '' }}" name="account" type="text" class="form-control" placeholder="00000000-00">
        </div>
        <div class="col-md-3">
          <label class="form-label">Chave PIX</label>
          <input value="{{ $person->pix_key ?? '' }}" name="pix_key" type="text" class="form-control">
        </div>
      </div>
    </div>
  </div>
...
<select required name="id_department" id="department-select" class="form-select">
  <option value="">Selecionar</option>
  @foreach($departments as $department)
    <option value="{{ $department->id_department }}" {{ $department->id_department == $person->department ? 'selected' : '' }}>
  ...
  @endforeach
        ...
        <div class="col-md-3">
          ...
          <select required name="role" class="form-select" id="job-select">
    ...
    <option value="">Selecionar</option>
    <option value="clt" {{ (isset($person->contract_type) && $person->contract_type == 'clt') ? 'selected' : '' }}>CLT (Contrato de Trabalho)</option>
    <option value="pj" {{ (isset($person->contract_type) && $person->contract_type == 'pj') ? 'selected' : '' }}>PJ (Pessoa Jurídica)</option>
    <option value="temporario" {{ (isset($person->contract_type) && $person->contract_type == 'temporario') ? 'selected' : '' }}>Temporário</option>
    <option value="estagio" {{ (isset($person->contract_type) && $person->contract_type == 'estagio') ? 'selected' : '' }}>Estágio</option>
    <option value="freelancer" {{ (isset($person->contract_type) && $person->contract_type == 'freelancer') ? 'selected' : '' }}>Freelancer</option>
    <option value="aprendiz" {{ (isset($person->contract_type) && $person->contract_type == 'aprendiz') ? 'selected' : '' }}>Aprendiz</option>
    <option value="autonomo" {{ (isset($person->contract_type) && $person->contract_type == 'autonomo') ? 'selected' : '' }}>Autônomo</option>
</select>

        </div>
        <div class="col-md-3">
          <label class="form-label">Data de Admissão</label>
          <input  value="{{ $person->admission_date ?? '' }}"  name="admission_date" type="date" class="form-control">
        </div>

        <div class="col-md-3">
          <label class="form-label">Matrícula</label>
          <input  value="{{ $person->registration_number ?? '' }}"  name="registration_number" type="text" class="form-control">
        </div>
        <div class="col-md-3">
          <label class="form-label">Período de Experiência</label>
          <input  value="{{ $person->experience_period ?? '' }}"  name="experience_period" type="text" class="form-control">
        </div>
        <div class="col-md-3">
          <label class="form-label">Data Contrato</label>
          <input  value="{{ $person->contract_date ?? '' }}" name="contract_date" type="date" class="form-control">
        </div>
        <div class="col-md-3">
          <label class="form-label">Vencimento Contrato</label>
          <input  value="{{ $person->contract_expiration_date ?? '' }}"  name="contract_expiration_date" type="date" class="form-control">
        </div>
      </div>
    </div>
  </div>
...
$(document).ready(function () {
  ...
  $('a[href]:not([href^="javascript:void(0)"]), a[href="/employee/profile/{{ $person->cpf ?? '' }}"], .btn-outline-danger').on('click', function (e) {
  if (edited) {
    e.preventDefault();
